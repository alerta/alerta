...
class TagsAppendTestCase(unittest.TestCase):
    ...
    def test_tag_alert(self):
        ...
        data = json.loads(response.data.decode('utf-8'))
        self.assertIn(alert_id, data['alert']['id'])
        self.assertEqual(sorted(data['alert']['tags']), sorted(['foo', 'bar', 'baz', 'quux', 'xyz']))
        ...
        data = json.loads(response.data.decode('utf-8'))
        self.assertIn(alert_id, data['alert']['id'])
        self.assertEqual(sorted(data['alert']['tags']), sorted(['foo', 'bar', 'baz', 'xyz']))

        del plugins.plugins['tag']


class TagsNoAppendTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_config = {
            'TESTING': True,
            'AUTH_REQUIRED': False,
            'CORRELATE_APPEND_TAGS': False,
            'DUPLICATE_APPEND_TAGS': False,
            'PLUGINS': []
        }
        self.app = create_app(test_config)
        self.client = self.app.test_client()
    ...
    def test_tag_alert(self):
        ...
        data = json.loads(response.data.decode('utf-8'))
        self.assertIn(alert_id, data['alert']['id'])
        self.assertEqual(sorted(data['alert']['tags']), sorted(['foo', 'xyz']))
