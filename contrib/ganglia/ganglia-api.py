...
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(name)s[%(process)d] %(levelname)s - %(message)s", filename=LOGFILE)
global logger
...
class ApiMetric:
    ...
    def api_dict(self):
        ...
        metric = {'environment': self.environment,
                ...
                'units': units,
                'type': type,
                'sampleTime': datetime.datetime.fromtimestamp(int(self.host.reported)+int(self.host.tn)-int(self.tn)).isoformat()+".000Z",
    ...
    def parse_tags(tag_string):
        if tag_string is None: return None
        else:
    ...
    def __str__(self):
        return "%s %s %s %s %s %s" % (self.environment,self.grid.name,self.cluster.name,self.host.name,self.group,self.name)
...
class GangliaConfig:
    ...
    def parse_ganglia_config(self):
        ...
        for file in glob.glob(os.path.join(GangliaConfig.GANGLIA_PATH,'gmetad-*-*.conf')):
            ...
            logger.info('Found gmetad-%s-%s.conf with ports %d and %d', environment, service, ports.xml_port, ports.interactive_port)
...
class GangliaPollThread(Thread):
    ...
    def update_ganglia_data(self):
        ...
        for counter, gmetad in enumerate(gmetad_list):
            ...
            logger.debug("  (%d/%d) updated %d metrics for %s %s", counter+1, len(gmetad_list), metrics_for_gmetad, gmetad.environment, gmetad.service)
            time.sleep(0.2)
