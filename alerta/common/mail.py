...
class Mailer(object):
    ...
    def __init__(self, alert):
        ...
        text += 'Status: %s\n' % alert.status
        text += 'Text: %s\n' % alert.text
        if hasattr(alert, 'threshold_info'):
            text += 'Threshold Info: %s\n' % alert.threshold_info
        if hasattr(alert, 'duplicate_count'):
            text += 'Duplicate Count: %s\n' % alert.duplicate_count
        if hasattr(alert, 'more_info'):
            text += 'More Info: %s\n' % alert.more_info
        text += 'Historical Data\n'
        if hasattr(alert, 'graph_urls'):
            for g in alert.graph_urls:
                text += '%s\n' % g
        text += 'Raw Alert\n'
        text += '%s\n' % alert.get_body()
        ...
        html += '<table>\n'
        html += '<tr><td><b>Alert ID:</b></td><td><a href="http://%s:%s/alerta/widgets/v2/details?id=%s" target="_blank">%s</a></td></tr>\n' % (
            CONF.api_host, CONF.api_port, alert.get_id(), alert.get_id())
        ...
        if hasattr(alert, 'more_info'):
            html += '<tr><td><b>More Info:</b></td><td><a href="%s">see also</a></td></tr>\n' % alert.more_info
        html += '</table>\n'
        html += '</td></tr>\n'
        ...
        if hasattr(alert, 'graph_urls'):
            self.graph_cid = dict()
            for g in alert.graph_urls:
                self.graph_cid[g] = str(uuid.uuid4())
                html += '<tr><td><img src="cid:' + self.graph_cid[g] + '"></td></tr>\n'
    ...
    def send(self, mail_to=None):
        ...
        msg_img = dict()
        for g in self.graph_urls:
            try:
                image = urllib2.urlopen(g).read()
                msg_img[g] = MIMEImage(image)
                LOG.debug('graph cid %s', self.graph_cid[g])
                msg_img[g].add_header('Content-ID', '<' + self.graph_cid[g] + '>')
        ...
        try:
            ...
            if CONF.debug:
                mail_exchange.set_debuglevel(True)
            mail_exchange.sendmail(CONF.mail_user, mail_to, msg_root.as_string())
            mail_exchange.quit()
