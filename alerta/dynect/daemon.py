...
class DynectDaemon(Daemon):
    ...
    def run(self):
        ...
        LOG.info('Starting DynECT GSLB monitor...')
        ...
        while True:
            try:
                ...
                LOG.info('Accept connection from client %s' % str(addr))
    ...
    def run(self):
        while True:
            ...
            for c in conn:
                try:
                    c.sendall(alert)
                    LOG.info('Sending %s' % alert)
                    LOG.info('alert sent to %s' % c)
                except:
                    LOG.info('Problem sending alert. No client ready to receive.')
...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            for item in info:
                ...
                if last[item][0] != info[item][0] or count == repeats:
                    ...
                    if item.startswith('gslb-'):
                        ...
                        LOG.info('GSLB state change from %s to %s' % (info[item][0], last[item][0]))
                        ...
                        LOG.info('Pool state change from %s to %s' % (info[item][0], last[item][0]))
                    ...
                    LOG.info('%s : Alert sent to client' % alertid)
            ...
            LOG.info('Sleeping for %s secs.' % _check_rate)
...
def init_config():
    ...
    LOG.info('Loaded %d config OK', len(config))
...
def queryDynect():
    ...
    LOG.info('Quering DynECT to get the state of GSLBs')
    ...
    try:
        ...
        LOG.info('GSLBs and Pools: %s', json.dumps(info))
