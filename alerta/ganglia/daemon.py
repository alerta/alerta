                ...
                for rule in rules:
                    ...
                    for m in response:
                        ...
                        if m['metric'] in rule['value']:
                            ...
                            if '$cluster' in rule['resource'] and 'graphUrl' in m:
                                metric[resource]['moreInfo'] = '/'.join(m['graphUrl'].rsplit('/', 2)[0:2]) + '/?c=%s' %
                                                               m['cluster']
                    ...
                    for resource in metric:
                        ...
                        try:
                            ...
                            logging.warning('Could not calculate %s value for %s because %s is not being reported',
                                            rule['event'], resource, rule['value'])
                            continue
                        except (SyntaxError, NameError):
                            logging.error('Could not calculate %s value for %s => eval(%s)', rule['event'], resource,
                                          metric[resource]['value'])
                            continue
                        except ZeroDivisionError:
                            logging.debug(
                            ...
                            logging.error('Could not calculate %s value for %s => eval(%s) (threw unknown exception)',
                                          rule['event'], resource, metric[resource]['value'])
                            continue
                        ...
                        for ti in metric[resource]['thresholdInfo']:
                            ...
                            try:
                                ...
                                logging.error('Could not evaluate %s threshold for %s => eval(%s)', rule['event'],
                                              resource, rule_eval)
                                result = False
                            ...
                            if result:
                                ...
                                logging.debug('calculated_value = %s, currentState = %s, currentCount = %d',
                                ...
                                logging.debug('Send alert if prevSev %s != %s AND thresh %d == %s',
                                              previousSeverity[(resource, rule['event'])], sev,
                                              currentCount[(resource, rule['event'], sev)], rule.get('count', 1))
                                logging.debug('Repeat? %s (%d - %d %% %d)', repeat,
                                              currentCount[(resource, rule['event'], sev)], rule.get('count', 1),
                                              rule.get('repeat', 1))
                                ...
                                if ((previousSeverity[(resource, rule['event'])] != sev and currentCount[
                                    ...
                                    logging.debug('%s %s %s %s rule fired %s %s %s %s',
                                    ...
                                    alert['value'] = "%s%s" % (
                                    calculated_value, format_units(metric[resource]['units']))
                                    alert['severity'] = sev
                                    ...
                                    ','.join(metric[resource]['environment']), sev, rule['event'], alert['value'],
                                    ','.join(metric[resource]['service']), resource)
