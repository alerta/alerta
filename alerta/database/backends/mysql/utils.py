...
Query.__new__.__defaults__ = ('1=1', {}, '(NULL)', 'status')  # type: ignore
    ...
    def filter_query(params, valid_params, query, qvars):
        ...
        for field in params.keys():
            ...
            if field in ['service', 'tags', 'roles', 'scopes']:
                query.append('AND JSON_OVERLAPS(`{0}`, %({0})s)'.format(column))
                qvars[column] = json.dumps(value)
            elif field.startswith('attributes.'):
                column = field.replace('attributes.', '')
                # attr_column = 'attr_' + column.replace('-', '_')
                query.append(f'AND JSON_CONTAINS(`attributes`, %({column})s)')
                qvars[column] = json.dumps({column: value[0]})
            elif len(value) == 1:
                value = value[0]
                if field.endswith('!'):
                    if value.startswith('~'):
                        # query.append('AND NOT LOWER(`{0}`) LIKE LOWER(:not_{0})'.format(column))
                        query.append('AND NOT LOWER(`{0}`) LIKE LOWER(%(not_{0})s)'.format(column))
                    ...
                    if value.startswith('~'):
                        query.append('AND LOWER(`{0}`) LIKE LOWER(%({0})s)'.format(column))
                ...
                if field.endswith('!'):
                    if '~' in [v[0] for v in value]:
                        query.append('AND LOWER(`{0}`) NOT REGEXP %(not_regex_{0})s'.format(column))
                        qvars['not_regex_' + column] = '|'.join([v.lstrip('~') for v in value])
                    else:
                        query.append('AND NOT `{0}` MEMBER OF(%(not_{0})s)'.format(column))
                        qvars['not_' + column] = json.dumps(value)
                else:
                    if '~' in [v[0] for v in value]:
                        query.append('AND LOWER(`{0}`) REGEXP %(regex_{0})s'.format(column))
                        qvars['regex_' + column] = '|'.join([v.lstrip('~') for v in value])
                    else:
                        query.append('AND `{0}` MEMBER OF(%({0})s)'.format(column))
                        qvars[column] = json.dumps(value)
...
class Alerts(QueryBuilder):
    ...
    def from_params(params: MultiDict, customers=None, query_time=None):
        ...
        # customer
        if customers:
            query.append('AND customer MEMBER OF(%(customers)s)')
            qvars['customers'] = json.dumps(customers)
        ...
        if len(ids) == 1:
            ...
            query.append('AND (id REGEXP LOWER(%(regex_id)s) OR last_receive_id REGEXP LOWER(%(regex_id)s))')
    ...
    def from_params(params: MultiDict, customers=None, query_time=None):
        ...
        if status:
            ...
            if BlackoutStatus.Active in status:
                squery.append('(start_time <= UTC_TIMESTAMP(3) AND end_time > UTC_TIMESTAMP(3))')
            if BlackoutStatus.Pending in status:
                squery.append('start_time > UTC_TIMESTAMP(3)')
            if BlackoutStatus.Expired in status:
                squery.append('end_time <= UTC_TIMESTAMP(3)')
            if squery:
    ...
    def from_params(params: MultiDict, customers=None, query_time=None):
        ...
        if status:
            ...
            if ApiKeyStatus.Active in status:
                squery.append('expire_time >= UTC_TIMESTAMP(3)')
