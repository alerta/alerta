    ...
    def __repr__(self):
        if 'singleterm' in self.tokens:
            if self.tokens.fieldname == '_exists_':
                return f'`attributes` ->\'$.{self.tokens.singleterm}\' IS NOT NULL'
            elif self.tokens.fieldname in ['correlate', 'service', 'tags']:
                return f'JSON_CONTAINS(`{self.tokens.field[0]}`, JSON_QUOTE(\'{self.tokens.singleterm}\'))'
            elif self.tokens.attr:
                tokens_attr = self.tokens.attr.replace('_', 'attributes')
                return f'LOWER(`{tokens_attr}` ->\'$.{self.tokens.fieldname}\') LIKE LOWER(\'%%{self.tokens.singleterm}%%\')'
            else:
                return f'LOWER(`{self.tokens.field[0]}`) LIKE LOWER(\'%%{self.tokens.singleterm}%%\')'
        if 'phrase' in self.tokens:
            if self.tokens.field[0] == '__default_field__':
                return f"LOWER(`__default_field__`) REGEXP '\\\\b{self.tokens.phrase}\\\\b'"
            elif self.tokens.field[0] in ['correlate', 'service', 'tags']:
                return f'JSON_CONTAINS(`{self.tokens.field[0]}`, JSON_QUOTE(\'{self.tokens.phrase}\'))'
            elif self.tokens.attr:
                tokens_attr = self.tokens.attr.replace('_', 'attributes')
                return f'LOWER(`{tokens_attr}` ->\'$.{self.tokens.fieldname}\') REGEXP \'\\\\b{self.tokens.phrase}\\\\b\''
            else:
                return f'LOWER(`{self.tokens.field[0]}`) REGEXP \'\\\\b{self.tokens.phrase}\\\\b\''
        if 'wildcard' in self.tokens:
            return f'LOWER(`{self.tokens.field[0]}`) REGEXP \'\\\\b{self.tokens.wildcard}\\\\b\''
        if 'regex' in self.tokens:
            return f'LOWER(`{self.tokens.field[0]}`) REGEXP \'{self.tokens.regex}\''
