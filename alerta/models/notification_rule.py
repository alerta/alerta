...
class NotificationRule:
    def __init__(self, environment: str, channel_id: str, receivers: List[str], **kwargs) -> None:
        if not environment:
    ...
    def from_document(cls, doc: Dict[str, Any]) -> 'NotificationRule':
        return NotificationRule(
            ...
            receivers=doc.get('receivers') or doc['toNumbers'],
    ...
    @staticmethod
    def find_by_id(id: str, customers: List[str] = None) -> Optional['NotificationRule']:
        return NotificationRule.from_db(db.get_notification_rule(id, customers))

    @staticmethod
    def find_all(query: Query = None, page: int = 1, page_size: int = 1000) -> List['NotificationRule']:
        return [NotificationRule.from_db(notification_rule) for notification_rule in db.get_notification_rules(query, page, page_size)]
