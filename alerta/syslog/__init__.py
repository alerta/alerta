...
#TODO(nsatterl): replace with kombu stuff
...
class SyslogDaemon(Daemon):
    ...
    def run(self):
        ...
        while not self.shuttingdown:
            try:
                ...
                ip, op, rdy = select.select([udp, tcp], [], [], 30)
                for i in ip:
                    if i == udp:
                        data = udp.recv(4096)
    ...
    def parse_syslog(self, data):
        ...
        # Decode syslog PRI
        facility = PRI >> 3
        facility = syslog.SYSLOG_FACILITY_NAMES[facility] # TODO(nsatterl): another mapping fn
        level = PRI & 7
        level = syslog.priority_to_code(level)
